...
#define _DIST_MAX  200.0   // maximum distance 250mm
...
void setup()
{
  ...
  Serial.begin(115200);    // 1,000,000 bps
}
...
void loop()
{
  ...
  if(100<=dist_raw && dist_raw<=200){
    ...
    if(dist_raw < 100) dist_raw = 100;
    if(dist_raw > 200) dist_raw = 200;
    digitalWrite(PIN_LED, HIGH);
  }
   // Put range Filter code here (_DIST_MIN ~ _DIST_MAX)
  // and turn on LED if the distance is in the range 
  dist_ema = dist_raw * EMA_ALPHA + dist_ema * (1 - EMA_ALPHA); // Put EMA filter code here
